...
class WordSearcherSpider(CrawlSpider):
    ...
    searched_words = ["Virtue", "signalling", "is", "society", "version", "of", "Proof", "Stake"]
    found_words = []
    rules = [
    ...
    def detail(self, response):
        for word in self.searched_words:
                if word.lower() not in self.found_words:
                    if word.lower() in str(response.body):
                        print("Found word {0} at url {1}".format(word.lower(), response.url))
